...
public class CloseSubscriptionHandler extends BaseHandler {
    final SubscriptionChannelManager subChannelMgr;
    // op stats
    final OpStats closesubStats;

    public CloseSubscriptionHandler(ServerConfiguration cfg, SnitchSeeker seeker,
    ...
    public void handleRequestAtOwner(final Snitch snitch, final PubSubRequest request, final Channel channel) {
        ...
        snitch.getSubscriptionManager().closeSubscription(topic, subscriberId, new Callback<Void>() {
            @Override
            public void operationFinished(Object ctx, Void result) {
                ...
                snitch.getDeliveryManager().stopServingSubscriber(topic, subscriberId, null,
                new Callback<Void>() {
                    @Override
