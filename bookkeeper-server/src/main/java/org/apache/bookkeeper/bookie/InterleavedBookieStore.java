 ...
 */
public class InterleavedBookieStore extends Thread implements BookieStore {
    ...
    private String getInstanceId(ZooKeeper zk) throws KeeperException, InterruptedException {
        ...
        try {
            byte[] data = zk.getData(conf.getZkLedgersRootPath() + "/" + BookKeeperConstants.INSTANCEID,
                    false, null);
    ...
    public InterleavedBookieStore(ServerConfiguration conf, ZooKeeper zk, StateTransistor transistor)
        ...
        super("InterleavedBookieStore-" + conf.getBookiePort());
        this.conf = conf;
