...
// The implementation is based following `WrappedBuffer` in netty.
// so use `io.netty.util.internal.StringUtil` as `WrappedBuffer`
...
/**
 * A ByteBufSideCar keeps references to referenced {@link ByteBuf} in a sidecar and releases then once the instance
 * itself is released.
 */
@Slf4j
public class ByteBufSideCar extends ByteBuf {

    private static final int INITIAL_LIST_SIZE = 1;

    public static ByteBufSideCar get(ByteBuf buffer) {
        ByteBufSideCar bb = RECYCLER.get();
        bb.buf = buffer;
        return bb;
    }

    private static final Recycler<ByteBufSideCar> RECYCLER = new Recycler<ByteBufSideCar>() {
        @Override
        protected ByteBufSideCar newObject(Handle<ByteBufSideCar> handle) {
            return new ByteBufSideCar(handle);
        }
    };

    private final Handle<ByteBufSideCar> handle;

    // main byte buf
    private ByteBuf buf;
    private final ArrayList<ReferenceCounted> sidecar;

    private ByteBufSideCar(Handle<ByteBufSideCar> handle) {
        this.handle = handle;
        this.sidecar = new ArrayList<>(INITIAL_LIST_SIZE);
    }

    public void addToSideCar(ReferenceCounted buf) {
         sidecar.add(buf);
    }

    private void releaseSideCar() {
        sidecar.forEach(ReferenceCounted::release);
        sidecar.clear();
    }
    ...
    public boolean release(int decrement) {
        ...
        try {
            released = buf.release(decrement);
            if (released) {
                releaseSideCar();
            }
            return released;
    ...
    //
    // ByteBuf Methods - following the implementation of {@link WrappedByteBuf} in Netty
    //
